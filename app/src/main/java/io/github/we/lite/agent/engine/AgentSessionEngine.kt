package io.github.we.lite.agent.engine

import io.github.we.lite.agent.data.entity.ApprovalStatus
import io.github.we.lite.agent.data.entity.ConditionalPromptEntity
import io.github.we.lite.agent.model.LlmClient
import io.github.we.lite.agent.model.LlmMessage
import io.github.we.lite.agent.model.LlmRole
import io.github.we.lite.agent.model.LlmStreamEvent
import io.github.we.lite.agent.model.LlmToolCall
import io.github.we.lite.agent.model.LlmToolSpec
import io.github.we.lite.agent.tool.ToolLoadingMode
import io.github.we.lite.agent.tool.ToolRegistry
import io.github.we.lite.agent.tool.WireTool
import io.github.we.lite.agent.ui.UiImageSink
...
class TurnConfig(
     ...
     */
    val toolVisibility: io.github.we.lite.agent.tool.ToolVisibility =
        io.github.we.lite.agent.tool.ToolVisibility.fromGlobals(),
    /**
...
class AgentSessionEngine(
    ...
    fun runTurn(
        ...
        try {
            ...
            while (true) {
                ...
                val request = io.github.we.lite.agent.model.LlmRequest(
    ...
    private fun teardownTurn() {
        runCatching { io.github.we.lite.agent.ui.UiAutomator.cancelActiveGesture() }
            .onFailure { WeLogger.w(TAG, "gesture teardown failed: ${it.message}") }
        runCatching { io.github.we.lite.agent.jvm.JvmObjectRegistry.purgeDead() }
    ...
    private suspend fun executeToolCall(
        ...
        visibility: io.github.we.lite.agent.tool.ToolVisibility,
        ...
        if (tool.mode == io.github.we.lite.agent.tool.ToolMode.MANUAL_APPROVAL) onAwaitingApproval(call.name)
        ...
        return when (decision) {
            is ApprovalDecision.Allowed -> {
                val status = when (tool.mode) {
                    io.github.we.lite.agent.tool.ToolMode.MANUAL_APPROVAL -> ApprovalStatus.USER_APPROVED
                    io.github.we.lite.agent.tool.ToolMode.SMART_APPROVAL -> ApprovalStatus.AI_APPROVED
    ...
    private fun parseArgs(argumentsJson: String): JsonObject =
        runCatching { io.github.we.lite.agent.model.LlmJson.json.parseToJsonElement(argumentsJson).jsonObject }
