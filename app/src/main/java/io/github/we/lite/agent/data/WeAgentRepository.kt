...
import io.github.we.lite.agent.data.WeAgentRepository.getExternalServiceKey
import io.github.we.lite.agent.data.WeAgentRepository.permissionCache
...
import io.github.we.lite.agent.data.entity.MessageRole
...
import io.github.we.lite.agent.model.LlmMessage
import io.github.we.lite.agent.model.LlmRole
import io.github.we.lite.agent.model.LlmToolCall
import io.github.we.lite.agent.tool.BuiltinToolProvider
import io.github.we.lite.agent.tool.ProviderKind
import io.github.we.lite.agent.tool.ToolMode
import io.github.we.lite.agent.tool.ToolPermissionSource
...
object WeAgentRepository : ToolPermissionSource {
     ...
     */
    suspend fun updateSessionUsage(id: String, usage: io.github.we.lite.agent.model.LlmUsage?) {
        ...
        toolCalls.forEach { tc ->
            ...
            db.toolCallDao().upsert(
                ToolCallEntity(
                    ...
                    resultJson = null,
                    approvalStatus = io.github.we.lite.agent.data.entity.ApprovalStatus.AUTO_ALLOWED,
                    approvalReason = null,
                    executedAt = null,
    ...
    suspend fun appendToolResult(
        ...
        resultText: String,
        status: io.github.we.lite.agent.data.entity.ApprovalStatus,
    ) {
    ...
    suspend fun deleteModelProvider(id: String) {
        ...
        io.github.we.lite.agent.model.ModelProviderManager.invalidate(id)
    }
    ...
    suspend fun upsertMcpProvider(provider: ProviderEntity) {
        ...
        io.github.we.lite.agent.mcp.McpClientManager.sync()
    }
    ...
    suspend fun upsertWorkspace(w: io.github.we.lite.agent.data.entity.WorkspaceEntity) =
    ...
    suspend fun observeWorkspacesOnce(): List<io.github.we.lite.agent.data.entity.WorkspaceEntity> =
    ...
    fun observeTriggers(): Flow<List<io.github.we.lite.agent.data.entity.TriggerEntity>> =
        db.triggerDao().observeAll()

    suspend fun getAllTriggersOnce(): List<io.github.we.lite.agent.data.entity.TriggerEntity> =
        db.triggerDao().getAllOnce()

    suspend fun getEnabledTriggers(): List<io.github.we.lite.agent.data.entity.TriggerEntity> =
        db.triggerDao().getEnabled()

    suspend fun getTrigger(id: String): io.github.we.lite.agent.data.entity.TriggerEntity? =
        db.triggerDao().getById(id)

    suspend fun getTriggersForSession(sessionId: String): List<io.github.we.lite.agent.data.entity.TriggerEntity> =
        db.triggerDao().getForSession(sessionId)

    suspend fun upsertTrigger(trigger: io.github.we.lite.agent.data.entity.TriggerEntity) =
