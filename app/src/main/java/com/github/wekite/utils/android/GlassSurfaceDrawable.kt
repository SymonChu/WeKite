...
class GlassSurfaceDrawable(
    ...
    /** 边缘细高光描边宽度 (px), 0 = 不描边 (对齐首页悬浮底栏 Highlight 的玻璃感)。 */
    ...
    /** 边缘细高光: 1dp 25% 白描边沿圆角走, 对齐首页悬浮底栏的玻璃高光观感。 */
    private fun drawStroke(canvas: Canvas, b: Rect) {
        if (cornerRadiusPx <= 0f || strokeWidthPx <= 0f) return
        strokePaint.strokeWidth = strokeWidthPx
        val inset = strokeWidthPx / 2f
        canvas.drawRoundRect(
            b.left + inset, b.top + inset,
            b.right - inset, b.bottom - inset,
            cornerRadiusPx, cornerRadiusPx, strokePaint
        )
    }
