...
> ⚠️ **项目状态：编译未成功（已暂停）**
>
> 1. **Zygisk 版本**：首次注入后微信卡在地球启动界面（未解决）
> 2. **LSPosed 版本**：DEX 缓存更新无限循环（扫描完成后重启仍重新扫描，未解决）
>
> 两个方向的启动问题均未根治，项目已暂停维护。**请勿刷入使用。**
> 代码保留仅供研究参考。

WeKite 是一套针对微信的模块化增强方案：核心功能全部围绕「不打扰、可开关、低开销」设计。
...
| 群员邀请者查询 | 查看成员是被谁拉进群的 |
| 已读追踪 | 本地 Hook 统计群消息已读人数（不依赖服务器） |

### 🎨 界面美化
| 功能 | 说明 |
|------|------|
| 莫奈引擎 | 动态取色，将自定义配色应用到微信原生组件 |
| 四 Tab 主题背景 | 为主页/通讯录/发现/我分别设置背景图 |
| 主题包导入导出 | 主题配置打包分享、导入恢复 |
| 自定义配色 | 个性化界面颜色方案 |
| 底部导航栏美化 | 自定义首页底部导航样式 |
| 对话框背景模糊 | 对话框窗口级毛玻璃效果 |
| 圆角气泡 | 聊天气泡圆角化 |
...
> ⚠️ **项目已暂停，暂无可用的正式版本。** 以下安装说明仅为历史记录，请勿使用。
