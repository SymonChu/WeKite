...
| 群聊消息分析 | 聊天页「N条新消息」胶囊旁挂同尺寸「AI分析」胶囊（胶囊在上方时挂它下面 12dp、在下方时挂它上面 12dp；胶囊不在时停在屏幕右侧居中），点击即分析**进会话时那批未读新消息**（进会话时没有未读才分析当天消息） |
...
| 自定义配色 | 个性化界面颜色方案 |
| 悬浮标题栏 | 聊天界面顶部标题栏及挂件改为悬浮卡片形式（建议搭配聊天界面沉浸） |
| 聊天界面沉浸 | 聊天界面 edge-to-edge，内容延伸到状态栏背后，消息可滚到状态栏下方 |
| 底部导航栏美化 | 自定义首页底部导航样式（支持 Telegram 风格悬浮底栏，下拉小程序面板时自动隐藏） |
| 圆角头像 | 将头像显示为圆角样式 |
...
| 加入群聊自动免打扰 | 加入新的群聊后自动开启消息免打扰 |
| 新好友自动备注 | 新增好友时按规则自动填写备注 |
...
| 朋友圈防撤回 | 好友删除的朋友圈仍可查看 |
| 评论防撤回 | 被删除的评论保留显示 |
| 自动点赞 | 一键/自动给朋友圈点赞，支持内容关键词包含/排除、分联系人单独设置 |
| 自动刷新 | 定时自动刷新朋友圈列表（间隔可配置） |
| 广告拦截 | 屏蔽朋友圈广告流 |
...
| 开屏广告移除 | 移除小程序开屏广告 |
| 视频广告移除 | 移除小程序视频广告 |
| 跳过激励广告 | 跳过小程序激励广告，展示后立即视为看完并发放奖励 |
